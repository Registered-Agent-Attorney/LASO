...
const char *table(RecordKind kind) {
  return table_names.at(static_cast<std::size_t>(kind));
}
...
void check_body_size(const Json &value) {
  if (value.dump().size() > std::size_t{4} * 1024 * 1024)
...
PostgresStorage::PostgresStorage(const std::string &dsn, const std::string &schema) {
  ...
  try {
    ...
    const auto lock = tx.exec_params(
        "SELECT pg_try_advisory_lock(hashtextextended(current_database() || "
    ...
    if (version == 0) {
      for (const auto name : table_names) {
        ...
        tx.exec("CREATE INDEX IF NOT EXISTS " + std::string(name) +
                "_run ON " + name + " (run_id, sequence)");
      }
...
void PostgresStorage::commit(const std::vector<Record> &records) {
  ...
  try {
    ...
    for (const auto &record : records) {
      const auto body = record.value.dump();
      check_body_size(record.value);
...
Json PostgresStorage::get(RecordKind kind, const std::string &id) const {
  ...
  try {
    pqxx::work tx(impl_->db);
    const auto result = tx.exec_params("SELECT body FROM " + std::string(table(kind)) +
                                          " WHERE id = $1",
                                      id);
    if (result.empty())
...
std::vector<Json> PostgresStorage::list(RecordKind kind, const std::string &run_id,
  ...
  try {
    ...
    pqxx::result result = run_id.empty()
                              ? tx.exec_params(sql, static_cast<long long>(limit),
