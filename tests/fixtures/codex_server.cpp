...
Json thread(const std::string &id, const std::string &cwd) {
  return Json{{"id", id}, {"cwd", cwd}, {"model", "fixture-model"},
              {"modelProvider", "fixture-provider"}};
}
} // namespace

int main(int argc, char **argv) {
  const std::string mode = argc > 1 && std::string(argv[1]) != "app-server"
                               ? argv[1]
                               : (std::getenv("LASO_CODEX_FIXTURE_MODE")
                                      ? std::getenv("LASO_CODEX_FIXTURE_MODE")
                                      : "success");
  std::string line;
  while (std::getline(std::cin, line)) {
    ...
    if (method == "thread/start") {
      ...
      const auto turn = Json{{"id", "fixture-turn"}, {"status", "inProgress"}};
      send(Json{{"jsonrpc", "2.0"}, {"id", id}, {"result", Json{{"turn", turn}}}});
      const auto item = Json{{"type", "agentMessage"},
                             {"text", prompt.find("continue") != std::string::npos
                                          ? "FIXTURE-CONTINUED"
                                          : "FIXTURE-COMPLETE"}};
      ...
      const auto completed_turn = Json{{"id", "fixture-turn"}, {"status", "completed"},
                                       {"durationMs", 4}};
      send(Json{{"method", "turn/completed"},
                {"params", Json{{"threadId", "fixture-session"},
                                  {"turn", completed_turn}}}});
