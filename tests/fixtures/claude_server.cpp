...
int main(int argc, char **argv) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    emit({{"type", "system"}, {"subtype", "init"}, {"session_id", session},
          {"cwd", std::filesystem::current_path().string()}, {"model", "fixture-model"},
    ...
    if (mode == "permission" || mode == "question") {
      ...
      emit({{"type", "control_request"},
            {"request_id", id},
            {"request", {{"subtype", subtype}, {"tool_name", "Bash"},
      ...
      if (answer.is_discarded() || answer.value("type", std::string{}) != "control_response")
        return 67;
      const auto response = answer.value("response", Json::object()).value("response", Json::object());
      if (mode == "permission" && response.value("behavior", std::string{}) != "allow") {
        emit({{"type", "result"}, {"subtype", "error_during_execution"},
              {"is_error", true}, {"session_id", session}, {"result", "permission denied"}});
        return 0;
      }
    }
    if (mode == "failure") {
      emit({{"type", "result"}, {"subtype", "error_during_execution"},
            {"is_error", true}, {"session_id", session}, {"result", "fixture failure"}});
      return 0;
    }
    ...
    emit({{"type", "result"}, {"subtype", "success"}, {"is_error", false},
          {"duration_ms", 12}, {"session_id", session}, {"result", "CLAUDE_DONE"},
