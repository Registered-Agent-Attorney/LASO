...
TEST(Workspace, ManifestStagesOnlyBoundedRelativeFilesWithIntegrity) {
  ...
  const auto staged = stage_workspace(manifest, directory.path / "staging", "run", "work", "attempt");
...
TEST(Storage, PostgresUpgradesSchemaSevenToCurrent) {
  ...
  const std::vector<std::string> prior_tables = {
      "pipelines",          "runs",          "attempts",       "messages",
      "approvals",          "artifacts",     "events",         "schedules",
      "triggers",           "schedule_occurrences", "trigger_deliveries",
      "event_sources",      "external_event_claims", "worker_jobs",
      "worker_interactions"};
  try {
    ...
    for (const auto &name : prior_tables)
      transaction.exec("CREATE TABLE \"" + name + "\" (id TEXT PRIMARY KEY, run_id TEXT NOT NULL, "
    ...
    transaction.exec("CREATE TABLE laso_instances (instance_id TEXT PRIMARY KEY, "
